...
import {
  LayoutDashboard, Megaphone, Users, BarChart2, Zap, Trophy,
  PlusCircle, Settings, LogOut, Shield, TrendingUp, Wallet,
  ClipboardList, UserCheck, Star, Target,
} from 'lucide-react';
...
export default function Sidebar({ className = '' }) {
  ...
  const nav = user?.role==='creator' ? CREATOR_NAV : user?.role==='brand' ? BRAND_NAV : ADMIN_NAV;
  const roleColor = user?.role==='admin'?'var(--gold)':user?.role==='brand'?'var(--acc)':'var(--p2)';
    ...
    <nav className={`sidebar ${className}`.trim()}>
      ...
      <div style={{ padding:'18px 16px 14px', borderBottom:'1px solid var(--border)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:9 }}>
          <div style={{ width:30, height:30, borderRadius:8, background:'linear-gradient(135deg,var(--p),var(--acc))',
            display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, flexShrink:0 }}>⚡</div>
          <div>
            <div style={{ fontFamily:'var(--fd)', fontWeight:800, fontSize:15, color:'var(--t1)', lineHeight:1 }}>Creatokite</div>
            <div style={{ fontSize:9, color:roleColor, marginTop:2, textTransform:'uppercase', letterSpacing:1 }}>
              {user?.role==='admin'?'Control Center':user?.role==='brand'?'Brand Portal':'Creator Studio'}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding:'12px 10px', borderTop:'1px solid var(--border)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:9, padding:'8px', borderRadius:'var(--r)',
        ...
        <button onClick={handleLogout} className="btn btn-ghost btn-sm w-full" style={{ justifyContent:'flex-start', color:'var(--rose)' }}>
